...
// Read data from the control file
  !! ad_comm::change_datafile_name("goa_dr_2018.ctl");    //Read in phases, penalties and priors from "tem.ctl"
...
PARAMETER_SECTION
 // Stock-recruitment
  ...
  number 	log_avgfishsel;	

  //Maturity
  init_bounded_number a50_mat(0,15,1)
  init_number delta_mat(1)

  vector L_pmat(1,nages_mat)
  vector C_pmat(1,nages_mat)
  //sdreport_vector Pred_pmat(1,nages_mat)
 ...
 // Longline Survey selectivity
  ...
  vector 	log_srv2_sel(1,nages_M);							// vector of survey selectivy log parameters including those not estimated
...
//   Selectivity does not change for ages greater than n_fish_sel_ages
  if (fsh_sel_opt==1)
  {
    ...
    for (j=n_fish_sel_ages+1;j<=nages_M;j++)
  ...
  {
    for (j=1;j<=nages_M;j++)
      fish_sel(j) = 1./(1. + mfexp(-2.944438979*((j+3)-a50)/delta));
  }
...
//  Survey selectivity set equal to fishery selectivity
  ...
  else //  Estimate survey and fishery selectivity separately
  {
    if (srv1_sel_opt==1)
    {
      ...
      for (j=n_srv1_sel_ages+1;j<=nages_M;j++)
    ...
    {
      for (j=1;j<=nages_M;j++)
        srv1_sel(j) = 1./(1. + mfexp(-2.944438979*((j+3)-a50_srv1)/delta_srv1));
    }
  }
//  Longline survey selectivity set equal to fishery selectivity
  ...
  else //  Estimate survey and fishery selectivity separately
  {
    if (srv2_sel_opt==1)
    {
      ...
      for (j=n_srv2_sel_ages+1;j<=nages_M;j++)
    ...
    {
      for (j=1;j<=nages_M;j++)
        srv2_sel(j) = 1./(1. + mfexp(-2.944438979*((j+3)-a50_srv2)/delta_srv2));
    }
  }
...
FUNCTION get_prop_mat
  ...
  for (int i=1;i<=nages_mat;i++){
   ...
   Pred_pmat(i) = 1/(1+exp(-1.0*delta_mat*(i-a50_mat)));}

  for (int i=1;i<=nages_M;i++){
  //p_mature(i) = value(Pred_pmat(i));

  p_mature(i) = 1/(1+exp(-1.0*delta_mat*((i+3)-a50_mat)));
...
// Start year
  if (SrType==3)
  {
    ...
    natage(styr,nages_M)      = mfexp(log_mean_rec - natmort * (nages_M-1)) 
                              / (1. - exp(-natmort) );
  }
