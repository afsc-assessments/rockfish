...
DATA_SECTION //------------------------------------------------------------------------------------------------
  ...
  !! cout << priorsdfisha50dev << endl;
  init_vector aibiomass2016(1974,2016)
  !! cout << aibiomass2016 << endl;
  init_vector ebsbiomass2016(1974,2016)
  !! cout << ebsbiomass2016 << endl;
  ...
  vector cv_srv3(1,nyrs_srv3)	// vector of the cv's for the AI  survey
 ...
 END_CALCS
  ...
  init_int har_flag 
  !! cout <<" har_flag is " << har_flag << endl; 
  ...
  init_vector lambda(1,18)
  !! cout << lambda << endl;
  //vector fish_unbiased_ac_samp(1,nyrs_fish_unbiased_ac)
  //vector fish_lc_samp(1,nyrs_fish_lc)
  //vector surv3_unbiased_ac_samp(1,nyrs_surv3_unbiased_ac)   
  //vector surv3_lc_samp(1,nyrs_surv3_lc)
  //vector surv_ebs_unbiased_ac_samp(1,nyrs_surv_ebs_unbiased_ac)
  //vector surv_ebs_lc_samp(1,nyrs_surv_ebs_lc)
 ...
 LOCAL_CALCS  // set phases for fishery and survey selectivity options, and time-varying deviations
  ...
  if (lambda(16)==0 & lambda(17)==0 )  // turn off ebs srv sel paramaters if the age and length comps are not being fit 
    phase_s_sel_param_ebs = -1;
  if (lambda(18)==0)  // turn off ebs srv catchability if it is not being fit
    phase_q_srv_ebs = -1;
...
INITIALIZATION_SECTION //-------------------------------------------------------------------------------------
  ...
  sel_aslope_forfish 0.35 //5.0 // 0.35
  //slopedev 0.0
  sel_dslope_forfish 0.0
  sel_aslope_srv3 0.25
  sel_aslope_srv_ebs 0.25
  //sel_dslope_srv3 -0.246474
  sel_a50_domfish 6.574314
  sel_d50_domfish 24.903941
  sel_a50_forfish 19.0 //30.0 //19.0
  //a50dev 0.0
  sel_d50_forfish 24.903941
  sel_a50_srv3 20.0 //0.0 // 20.0
  sel_a50_srv_ebs 20.0
  //sel_d50_srv3 10.50689
  steepness 0.5
  historic_F 0.000
  q_srv3     1.0
  q_srv_ebs 1.0
...
PARAMETER_SECTION //-----------------------------------------------------------------------------------------
 ...
 init_bounded_number sel_aslope_forfish(0.1,3.0,phase_f_sel_ascend)  
 //number sel_aslope_forfish
 init_number sel_dslope_forfish(phase_f_sel_descend)
 init_number sel_a50_domfish(-1) 
 init_number sel_d50_domfish(-1)
 init_bounded_number sel_a50_forfish(0.1,30.0,phase_f_sel_ascend)
 //number sel_a50_forfish
 ...
 //init_number a50dev(phase_logist_param)    // survey sel parameters
   ...
   // now the logistic curve parameters for the trawl survey (AI)
 ...
 init_bounded_number sel_a50_srv3(0.1,30.0,phase_s_sel_param_ai)
  ...
  // now the logistic curve parameters for the trawl survey (ebs)
...
// The parameters for getting the predicted values for the AI trawl survey index (srv3)
  init_bounded_number q_srv3(.1,4.0,4)		// AI trawl survey  catchability
  vector pred_srv3(styr_fish,endyr_r)       // AI predicted survey biomass
  init_bounded_number q_srv_ebs(.1,4.0,phase_q_srv_ebs)    // EBS trawl survey  catchability
  vector pred_srv_ebs(styr_fish,endyr_r)       // EBS predicted survey biomass
...
PRELIMINARY_CALCS_SECTION //-------------------------------------------------------------------------------
 ...
 for ( i=1;i<=nyrs_surv_ebs_unbiased_ac_r; i++)  // EBS survey length comps
 {
...
//      future_projections();
     ...
     if (mceval_phase())
  {
    ...
    evalout << obj_fun<<" "<<q_srv3<<" "<<M<<" "<<totbiom<<" "<<sp_biom<<" "<<est_rec<<endl;
  } 
...
FUNCTION get_selectivity //---------------------------------------------------------------------------------
 ...
 //  sel_aslope_forfish = sel_aslope_srv3*exp(slopedev);  // allow the fish sel parameters to
 //  sel_a50_forfish = sel_a50_srv3*exp(a50dev);		// vary a little from the survey sel
...
FUNCTION age_likelihood  // fit the age comps--------------------------------------------------------------------
 ...
 obj_fun += (lambda(12)*age_like(1) + lambda(13)*age_like(2) + lambda(14)*age_like(3) + lambda(15)*age_like(4) + lambda(16)*age_like(5) + lambda(17)*age_like(6));
 ...
 for (i=1;i<=nyrs_surv3_unbiased_ac_r;i++)
 {
   ...
   for (j=1;j<=nages_dat;j++)
   {
     ...
     //sac_nr(k) = tmp1/sqrt(tmp2/(surv3_unbiased_ac_samp(i)*(4./6.)));
 ...
 {
   ii=yrs_surv_ebs_unbiased_ac_r(i);
   tmp3 = ages_dat*oac_surv_ebs_unbiased_r(i);
   tmp4 = ages_dat*eac_surv_ebs_unbiased_dat(ii);
...
FUNCTION prior  // compute the prior parts of the likelood for q_surv3, M, and the fish sel deviations --------------------------------------------------------------------
 ...
 prior_like(3) = square(log(M) - log(priormeanM) + square(priorcvM)/2.0)/(2.*square(priorcvM));
...
FUNCTION srv_likelihood  // fit to indices (lognormal) ---------------------------------------------------------
  ...
  for (i=1;i<=nyrs_srv_ebs_r;i++)  // EBS survey likelihood
 {
   ...
   rmse(3) += square(log(obs_srv_ebs(i)+1e-13) - log(pred_srv_ebs(ii)+1e-13));
   //rmse(1) += square(log(obs_srv3(i)+1e-13) - log(pred_srv3(ii)+1e-13));

   //survey_nr(i) = (log(obs_srv3(i)+1e-13) - log(pred_srv3(ii)+1e-13))/cv_srv3(i);
 }
  //rmse(1) = sqrt(rmse(1)/nyrs_srv3);
  rmse(3) = sqrt(rmse(3)/nyrs_srv_ebs_r);

  obj_fun+= lambda(2)*surv_like(1) + lambda(18)*surv_like(2);
...
FUNCTION update_compweights   // update if the comp is estmated, otherwise carry over previous comp weight
  ...
  if (lambda(15)>0 && nyrs_surv3_lc_r >0) 
   ...
   if (lambda(16)>0 && nyrs_surv_ebs_unbiased_ac_r >0) 
     {
   ...
   if (lambda(17)>0 && nyrs_surv_ebs_lc_r >0) 
     {
...
REPORT_SECTION //-------------------------------------------------------------------------------------------
 ...
 report << "the first year where we do the new projection is " << lastyr_rec_a10+1 << endl; 
  ...
  report << "Predicted ebs survey: years "<<styr_fish<<" to " << endyr_r << endl;
    ...
    for (i=1;i<=nyrs_surv_ebs_unbiased_ac_r; i++)
  {
    ...
    report << "the mean effective n by data type: 'fish_ac', 'fish_lc', 'AI_surv_ac', 'AI_surv_lc','ebs_surv_ac', 'ebs_surv_lc' " << endl;  
       ...
       report << "the sample weights by data type: 'fish_ac', 'fish_lc', 'AI_surv_ac', 'AI_surv_lc','ebs_surv_ac', 'ebs_surv_lc'" << endl;
        ...
        report << "the rmse for the survey and rec: 'AI', 'rec', 'ebs_srv'"<<endl;;
        report << rmse << endl;

	report << "The likelhood components: 'histFpen', 'selpen', 'rec_likelihood', 'AI_srv','EBS_srv','cat_likelihood', 'Fmort_pen','fish_ac','fish_lc', 'AI_surv_ac', 'AI_surv_lc','ebs_surv_ac', 'ebs_surv_lc'"<<endl;      
  report << hf_pen<<" "<<sum(sel_like)<<"  "<<sum(rec_like)<<"  "<<surv_like(1)<<" "<<surv_like(2)<<" "<<catch_like<<" "<<fpen<<" "<<age_like<<endl;
  report << "the sel_like vector" << endl;
  report << sel_like << endl;    
  report << "the prior components of the like: 'prior_qsrv', 'prior_m' " << endl;
        report << prior_like(1) <<" "<<prior_like(3)<< endl;
        report << " the weights for the fac, flc, sac, slc,ebs_sac, ebs_slc are "<< endl;
        report << compweightsnew_ta12<< endl; 
        report << "the sdnr for the survey, fac, flc, sac, slc, ebs_ac, ebs_lc are   "<< endl;

        report << std_dev(survey_nr);
        if(nyrs_fish_unbiased_ac_r>0)
            report <<" "<<std_dev(fac_nr);
        else 
            report <<" "<<0; 

        if(nyrs_fish_lc_r>0)
            report <<" "<<std_dev(flc_nr);
        else 
            report <<" "<<0;     

        if(nyrs_surv3_unbiased_ac_r>0)
            report <<" "<<std_dev(sac_nr);
        else 
            report <<" "<<0;

        if(nyrs_surv3_lc_r>0)
            report <<" "<<std_dev(slc_nr);
        else 
            report <<" "<<0;            

        if(nyrs_surv_ebs_unbiased_ac_r>0)
            report <<" "<<std_dev(ebs_sac_nr);
        else 
            report <<" "<<0;

        if(nyrs_surv_ebs_lc_r>0)
            report <<" "<<std_dev(ebs_slc_nr) << endl;
        else 
            report <<" "<<0 << endl;   
        ...
        report <<" the root mean square error for the fac, flc, sac, and slc, ebs_sac, ebs_slc are" << endl;
        report << al_rmse(1) <<" "<<al_rmse(2)<<" "<<al_rmse(3)<<" "<<al_rmse(4)<<" "<<al_rmse(5)<<" "<<al_rmse(6)<<endl;
        report << "The AI survey q, EBS survey q, and M are" << endl;
        report << q_srv3 <<" "<< q_srv_ebs<<" "<< M << endl;
        ...
        report << sel_par << endl;
        report << " "<< endl;
         //report << -fish_lc_samp_r(1)*(olc_fish_r(1))*log(1.e-13+olc_fish_r(1)) << endl;
        report << " est_rec"  << endl;
        report << est_rec << endl;
        ...
        # include "re-s-report_exp_r.cxx"   // ADMB code to write the S-compatible report
